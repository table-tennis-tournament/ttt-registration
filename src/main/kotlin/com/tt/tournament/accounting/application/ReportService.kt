package com.tt.tournament.accounting.application
...
private const val NUMBER_OF_PAGES_FOR_DOUBLE = 24
...
class ReportService(val playerRepository: PlayerRepository) {
    ...
    fun generateDoubleListsBytes(): ByteArray {
        val playersPerDiscipline = playerRepository.readPlayersForAllDisciplines()
        val resultList = playersPerDiscipline.map { (disciplineName, playerCount) ->
            val rowCount = calculateRowCount(playerCount)
            parseThymeleafTemplateForDoubleLists(disciplineName, rowCount)
        }
        return generatePdfBytesFromHtml(resultList)
    }

    private fun calculateRowCount(playerCount: Int): Int {
        val possibleDoubles = ceil(playerCount / 2.0).toInt()
        return ceil(possibleDoubles / 24.0).toInt() * NUMBER_OF_PAGES_FOR_DOUBLE
    }

    private fun parseThymeleafTemplateForDoubleLists(disciplineName: String, rowCount: Int): String {
        ...
        context.setVariable("rowCount", rowCount)
        return templateEngine.process("double-list", context)
    }
